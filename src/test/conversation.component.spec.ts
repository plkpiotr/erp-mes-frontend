...
describe('ConversationComponent', () => {
  ...
  it('should call emailService.fetchConversation()', () => {
    spyOn(component, 'fetchEmails').and.callThrough();
    spyOn(service, 'fetchConversation').and.callThrough();
    component.ngOnInit();
    expect(component.fetchEmails).toHaveBeenCalled();
    expect(service.fetchConversation).toHaveBeenCalled();
  });

  describe('When "Wyślij" button is clicked', () => {
    beforeEach(() => {
      spyOn(component, 'divideContent').and.callFake(() => {});
      spyOn(service, 'reply').and.callThrough();
      component.send();
    });

    it('should call emailService.reply()', () => {
      expect(component.divideContent).toHaveBeenCalled();

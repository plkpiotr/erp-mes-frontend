...
describe('ComplaintsComponent', () => {
  ...
  it('should call complaintService.fetchAllComplaints()', () => {
    spyOn(component, 'fetchComplaints').and.callThrough();
    spyOn(service, 'fetchAllComplaints').and.callThrough();
    component.ngOnInit();
    expect(component.fetchComplaints).toHaveBeenCalled();
    expect(service.fetchAllComplaints).toHaveBeenCalled();
  });
  ...
  describe('When "Zobacz" button is clicked', () => {
    ...
    it('should call router.navigate()', fakeAsync(() => {
      component.seeComplaint(1);
      expect(router.navigate).toHaveBeenCalled();
      tick(50);
      expect(location.path()).toBe('/complaints/1');
    }));
  });

  describe('When "Dodaj reklamacje" button is clicked', () => {
    beforeEach(() => {
      spyOn(router, 'navigate').and.callThrough();
    });
    ...
    it('should call router.navigate()', fakeAsync(() => {
      ...
      expect(router.navigate).toHaveBeenCalled();
      tick(50);
      expect(location.path()).toBe('/shop-service/add?service=complaint');
    }));
  });
});
